...
class M_cargo extends CI_Model
{
    ...
    public function getDataTableRequest()
    {
        ...
        if ($this->session->userdata('group') != '1200' && $this->session->userdata('group') != '1280') {
            ...
            $this->db->where("(npwp_requestor = '".$this->session->userdata('F_npwp')."' OR npwp_consignee = '".$this->session->userdata('F_npwp')."' OR npwp_notifyparty = '".$this->session->userdata('F_npwp')."')");
        }

        $cari = strtolower($_POST["search"]["value"]);
        if (isset($_POST["search"]["value"])) {
            $this->db->where("(lower(noreqdo) LIKE '%" . $cari . "%' OR lower(nobl) LIKE '%" . $cari . "%' OR lower(nodo) LIKE '%" . $cari . "%' OR lower(nama_requestor) LIKE '%" . $cari . "%' OR lower(kd_shippingline) LIKE '%" . $cari . "%')", NULL, FALSE);
        }
        ...
        if (isset($order)) {
            ...
            if ($order_column == "0") {
                $this->db->order_by('noreqdo', $order_dir);
            } else if ($order_column == "1") {
                $this->db->order_by('tglreqdo', $order_dir);
            } else if ($order_column == "2") {
                $this->db->order_by('nobl', $order_dir);
            } else if ($order_column == "3") {
                $this->db->order_by('tglbl', $order_dir);
            } else if ($order_column == "4") {
                $this->db->order_by('nama_requestor', $order_dir);
            } else if ($order_column == "5") {
                $this->db->order_by('kd_shippingline', $order_dir);
            } else if ($order_column == "6") {
                $this->db->order_by('uraian', $order_dir);
            }
        } else {
    ...
    public function getDataTableRelease()
    {
        ...
        if ($cargo_type == "kontainer") {
            $this->db->where("(cargo_type != '3' OR cargo_type IS NULL)", null, false); //YJX
        }
        if ($cargo_type == "nonkontainer") {
            $this->db->where("(cargo_type = '3')", null, false); //YJX
        }
        if ($nobl) {
        ...
        $cari = strtolower($_POST["search"]["value"]);
        if (isset($_POST["search"]["value"])) {
            $this->db->where("(lower(noreqdo) LIKE '%" . $cari . "%' OR lower(nobl) LIKE '%" . $cari . "%' OR lower(nodo) LIKE '%" . $cari . "%' OR lower(nama_requestor) LIKE '%" . $cari . "%' OR lower(kd_shippingline) LIKE '%" . $cari . "%' OR lower(kd_terminal) LIKE '%" . $cari . "%')", NULL, FALSE);
        }
        ...
        if (isset($order)) {
            ...
            if ($order_column == "1") {
                ...
                $this->db->order_by('nodo', $order_dir);
            } else if ($order_column == "6") {
                $this->db->order_by('tgldoawal', $order_dir);
            } else if ($order_column == "7") {
                $this->db->order_by('tgldoakhir', $order_dir);
            } else if ($order_column == "8") {
                $this->db->order_by('kd_terminal', $order_dir);
    ...
    public function getDataTableRequestSP2ikt_BK09092021()
    {
        $limit = $_POST['length'];
        $offset = $_POST['start'];
        $this->db->select('A.*,B.uraian');
        $this->db->from('nswdb1.tblreqsp2ikt_header A');
        $this->db->join('nswdb1.tbltab B', 'A.status_sp2=B.kdnsw AND B.kdtab = \'D4\'', 'left');
        $this->db->where("(status_sp2='334')", NULL, FALSE);
        if ($this->session->userdata('group') != '1200' && $this->session->userdata('group') != '1280') {
            $this->db->where('npwp_requestor', $this->session->userdata('F_npwp'));
        }
        $cari = strtolower($_POST["search"]["value"]);
        if (isset($_POST["search"]["value"])) {
            $this->db->where("(lower(nosp2_request) LIKE '%" . $cari . "%' OR lower(shipping_line) LIKE '%" . $cari . "%' OR lower(terminal_operator) LIKE '%" . $cari . "%')", NULL, FALSE);
        }
        ...
        if (isset($order)) {
            ...
            if ($order_column == "0") {
                $this->db->order_by('nosp2_request', $order_dir);
            } else if ($order_column == "1") {
                $this->db->order_by('wk_status_return', $order_dir);
            } else if ($order_column == "2") {
                $this->db->order_by('shipping_line', $order_dir);
            } else if ($order_column == "3") {
                $this->db->order_by('terminal_operator', $order_dir);
            } else if ($order_column == "4") {
                $this->db->order_by('uraian', $order_dir);
            }
        } else {
            $this->db->order_by('A.id_reqsp2ikt_header', 'DESC');
        }
    ...
    public function getDataTableRequestSP2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
        $this->db->join('tblreqsp2ikt_header tih', 'tih.id_reqsp2ikt_header = tim.id_reqsp2ikt_header', 'left');
        $this->db->join('tblreqdo_header th', 'th.id_reqdo_header = tim.id_reqdo_header', 'left');
        $this->db->join('tbltab t', 't.kdnsw = tih.status_sp2', 'left');
        $this->db->where("(tih.status_sp2='334')", NULL, FALSE);
        if ($this->session->userdata('group') != '1200' && $this->session->userdata('group') != '1280') {
            $this->db->where("'" . $this->session->userdata('F_npwp') . "' in ( th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty)");
        }

        $cari = strtolower($_POST["search"]["value"]);
        if (isset($_POST["search"]["value"])) {
            $this->db->where("(lower(nosp2_request) LIKE '%" . $cari . "%' OR lower(shipping_line) LIKE '%" . $cari . "%' OR lower(terminal_operator) LIKE '%" . $cari . "%' OR lower(nobl) LIKE '%" . $cari . "%')", NULL, FALSE);
        }
        ...
        if (isset($order)) {
            ...
            $this->db->order_by('tih.id_reqsp2ikt_header', 'DESC');
        }
    ...
    public function getDataTableReleaseSP2ikt_BK09092021()
    {
        $limit = $_POST['length'];
        $offset = $_POST['start'];
        $this->db->select('A.*,B.uraian');
        $this->db->from('nswdb1.tblreqsp2ikt_header A');
        $this->db->join('nswdb1.tbltab B', 'A.status_sp2=B.kdnsw AND B.kdtab = \'D4\'', 'left');
        $this->db->where("(status_sp2='440')", NULL, FALSE);
        if ($this->session->userdata('group') != '1200' && $this->session->userdata('group') != '1280') {
            $this->db->where('npwp_requestor', $this->session->userdata('F_npwp'));
        }
        $cari = strtolower($_POST["search"]["value"]);
    ...
    public function getDataTableReleaseSP2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
        ...
        $this->db->join('tblreqdo_header th', 'th.id_reqdo_header = tim.id_reqdo_header', 'left');
        $this->db->join('tbltab t', 't.kdnsw = tih.status_sp2', 'left');
        $this->db->where("(tih.status_sp2='440')", NULL, FALSE);
        if ($this->session->userdata('group') != '1200' && $this->session->userdata('group') != '1280') {
            $this->db->where("'" . $this->session->userdata('F_npwp') . "' in ( th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty)");
        }
    ...
    public function getFilteredData_request()
    {
        $this->db->select('A.*,B.uraian,C.uraian as shipping');
        $this->db->from('tblreqdo_header A');
        $this->db->join('tbltab B', 'A.statusreqdo=B.kdnsw AND B.kdtab = \'D4\'', 'left');
    ...
    public function getFilteredDataSP2ikt_BK13092021()
    {
        $limit = $_POST['length'];
        $offset = $_POST['start'];
        $this->db->select('A.*,B.uraian');
    ...
    public function getFilteredDataSP2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
    ...
    public function count_all_sp2ikt_BK13092021()
    {
        $this->db->select('count(id_reqsp2ikt_header)');
        $this->db->from('nswdb1.tblreqsp2ikt_header A');
        $this->db->join('nswdb1.tbltab B', 'A.status_sp2=B.kdnsw AND B.kdtab = \'D4\'', 'left');
        $this->db->where("(status_sp2='334')", NULL, FALSE);
    ...
    public function count_all_sp2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
    ...
    public function getFilteredDataReleaseSP2ikt_BK13092021()
    {
        $limit = $_POST['length'];
        $offset = $_POST['start'];
        $this->db->select('A.*,B.uraian');
    ...
    public function getFilteredDataReleaseSP2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
    ...
    public function count_all_Releasesp2ikt_BK13092021()
    {
        $this->db->select('count(id_reqsp2ikt_header)');
        $this->db->from('nswdb1.tblreqsp2ikt_header A');
        $this->db->join('nswdb1.tbltab B', 'A.status_sp2=B.kdnsw AND B.kdtab = \'D4\'', 'left');
    ...
    public function count_all_Releasesp2ikt()
    {
        $this->db->distinct();
        $this->db->select('th.nobl, tih.*, th.npwp_requestor, th.npwp_consignee, th.npwp_notifyparty, t.uraian');
        $this->db->from('tblreqsp2ikt_mappingdo tim');
    ...
    public function ExecRequest()
    {
        ...
        if ($_FILES['filepath_requestor']['name'] != "") {
            ...
            if ($this->upload->do_upload('filepath_requestor')) {
                ...
                $filenya = 'https://apps1.insw.go.id/upload/meta_requestor/' . $file_name;
                $postFields = array();
                $postFields['file'] = '@' . realpath($full_path);
                $curl = curl_init($this->config->item('meta_documents'));
                // $curl = curl_init("10.239.7.81/engMetadata/engine/file/updatemetadata");
                // $curl = curl_init("10.239.7.81/engMetadata/uploadFile");
                // curl_setopt($curl, curl_, value)
                curl_setopt(
                    $curl,
                    CURLOPT_HTTPHEADER,
                    array( //custom header for my api validation you can get it from $_SERVER["HTTP_X_PARAM_TOKEN"] variable
        ...
        if ($cargo_type == "1" || $cargo_type == "2") {
            ...
            $package_satuan = $this->input->post('package_satuan');
            $gross_weight_non = $this->input->post('gross_weight_non');
            $gross_weight_non_satuan = $this->input->post('gross_weight_non_satuan');
            $measurement_qty = $this->input->post('measurement_qty');
            $measurement_satuan = $this->input->post('measurement_satuan');
            $no_col_non = $this->input->post('numNonCont');
        }
        ...
        if (is_array($request)) {
            ...
            $sqlBykData = "SELECT nobl, nodo from tblreqdo_header where nobl='$no_bl' and nodo is not null and statusreqdo != '999'";
            ...
            if ($bykData > 0) {
                ...
                $checkRequest    = $this->db->get_where('tblreqdo_header', array('nobl' => $request['nobl'], 'kd_shippingline' => $request['kd_shippingline'], 'statusreqdo !=' => '999'))->num_rows();
                if ($checkRequest > 0) {
                    ...
                    if ($exec) {
                        ...
                        if ($cargo_type == "1" or $cargo_type == "2") {
                            ...
                            for ($i = 0; $i < sizeof($goods_desc); $i++) {
                                if ($goods_desc[$i] != "") {

                                    $seri = $i + 1;

                                    //$LastIdGoods  = $this->db->insert_id();

                                    $goods_detail = array(
                                        'id_reqdo_header' => $LastId,
                                        'bl_number'   => $no_bl,
                                        'id_goods' => $seri,
                                        'goods_desc' => $goods_desc[$i]
                                        //'id_goods_desc' => $LastIdGoods
                                    );

                                    $execDetailGoods = $this->db->insert('tblreqdo_goods', $goods_detail);

                                    $measurement_detail = array(
                                        'id_reqdo_header' => $LastId,
                                        'id_goods' => $seri,
                                        'package_qty' => $package_qty[$i],
                                        'package_uom' => $package_satuan[$i],
                                        'gross_qty' => $gross_weight_non[$i],
                                        'gross_uom' => $gross_weight_non_satuan[$i],
                                        'measurement_qty' => $measurement_qty[$i],
                                        'measurement_uom' => $measurement_satuan[$i],
                                        'bl_number'   => $no_bl

                                        //'id_goods_desc' => $LastIdGoods
                                    );

                                    $execDetailMeasurement = $this->db->insert('tblreqdo_measurement', $measurement_detail);
                                }
                            }
                        }
                        ...
                        for ($i = 0; $i < sizeof($jenis_dok); $i++) {
                            if ($jenis_dok[$i] != "" && $jenis_dok[$i] != "10") {
                                ...
                                if ($_FILES['filepath_dok']['name'][$i] != "") {
                                    ...
                                    $folder_path = "dokumen";
                                    $folderName = "../upload/" . $folder_path;
                                    $config5['upload_path']          = $folderName;
                                    $config5['allowed_types'] = 'xlsx|csv|xls';
                                    $config5['max_size']             = 2048000;
                                    $new_name = time() . $_FILES['filepath_dok']['name'][$i];
                                    $config5['file_name'] = $new_name;
                                    if (!is_dir($folderName)) {
                                        mkdir($folderName, 0777, true);
                                        chmod($folderName, 0777);
                                    }
                                    ...
                                    if ($this->upload->do_upload('filepath_dok[]')) {
                                        ...
                                        foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                                            ...
                                            for ($row = 1; $row <= $totalrow; $row++) {
                                                ...
                                                $no_equip = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
                                                // $sqlequip = "INSERT INTO tblreqdo_equipment (id_reqdo_header,bl_number,equipment_identification_number) values ('" . $LastId . "','" . $no_bl . "','".$no_equip."')";
                                                // $execequip = $this->db->query($sqlequip);
                                                if ($no_equip != "") {
                                                    $sqlequip = "INSERT INTO tblreqdo_equipment (id_reqdo_header,bl_number,equipment_identification_number) values ('" . $LastId . "','" . $no_bl . "','" . $no_equip . "')";
                                                    $execequip = $this->db->query($sqlequip);
                                                }
                                ...
                                $date_dok = date('Y-m-d', strtotime($tgl_dok[$i]));
                                $sqldok = "INSERT INTO tblreqdo_dok (id_reqdo_header,jenis_dok,no_dok,tgl_dok,filepath_dok,filepath_dok_meta) values ('" . $LastId . "','" . $jenis_dok[$i] . "','" . $no_dok[$i] . "','" . $date_dok . "','" . $path_dok . "','" . $path_dok . "')";
                                $execDok = $this->db->query($sqldok);
                            }
                        }
    ...
    public function EditRequest()
    {
        ...
        if ($_FILES['filepath_requestor']['name'] != "") {
            ...
            if ($this->upload->do_upload('filepath_requestor')) {
                ...
                if (!$response) {
                    // $response = curl_error($curl);
                    $request["filepath_requestor_meta"] = "";
                } else {
                    file_put_contents('../upload/meta_requestor/' . $file_name, $response);
                    $request["filepath_requestor_meta"] = $file_name;
                }
            ...
            if ($bykData > 0) {
                ...
                if ($exec) {
                    ...
                    if ($cargo_type != "3") {
                        ...
                        $execDelete = $this->db->delete('tblreqdo_goods', $where);
                        $execDelete = $this->db->delete('tblreqdo_measurement', $where);
                    }
                    ...
                    if ($execDelete) {
                        ...
                        if ($cargo_type != "3") {
                            ...
                            for ($i = 0; $i < count($goods_desc); $i++) {
                                if ($goods_desc != "") {
                                    ...
                                    $execDetailCon = $this->db->insert('tblreqdo_measurement', $measurement_detail);
                                }
                            }
                        }
                        ...
                        for ($i = 0; $i < sizeof($jenis_dok); $i++) {
                            if ($jenis_dok[$i] == "10") {
                                if ($_FILES['filepath_dok']['name'][$i] != "") {
                                    $whereEquip = array("id_reqdo_header" => $id_reqdo_header);
                                    $execDeleteEquip = $this->db->delete('tblreqdo_equipment', $whereEquip);
                                }
                            }
                        }
                        ...
                        for ($i = 0; $i < sizeof($jenis_dok); $i++) {
                            if ($jenis_dok[$i] != "-" && $jenis_dok[$i] != "10") {
                                    ...
                                    if ($this->upload->do_upload('filepath_dok[]')) {
                                        ...
                                        foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                                            ...
                                            for ($row = 1; $row <= $totalrow; $row++) {
                                                ...
                                                $no_equip = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
                                                if ($no_equip != "") {
                                                    $sqlequip = "INSERT INTO tblreqdo_equipment (id_reqdo_header,bl_number,equipment_identification_number) values ('" . $id_reqdo_header . "','" . $no_bl . "','" . $no_equip . "')";
                                                    $execequip = $this->db->query($sqlequip);
                                                }
                                                // $sqlequip = "INSERT INTO tblreqdo_equipment (id_reqdo_header,bl_number,equipment_identification_number) values ('" . $id_reqdo_header. "','" . $no_bl. "','".$no_equip."')";
                                                // $execequip = $this->db->query($sqlequip);
                                ...
                                $date_dok = date('Y-m-d', strtotime($tgl_dok[$i]));
                                $sqldok = "INSERT INTO tblreqdo_dok (id_reqdo_header,jenis_dok,no_dok,tgl_dok,filepath_dok,filepath_dok_meta) values ('" . $id_reqdo_header . "','" . $jenis_dok[$i] . "','" . $no_dok[$i] . "','" . $date_dok . "','" . $path_dok . "','" . $path_dok . "')";
                                $execDok = $this->db->query($sqldok);
                            }
                        }
                    }
    ...
    public function ExecExtend()
    {
        ...
        $shipping_line  = $request['kd_shippingline'];
        $sql_func       = "SELECT fgetnoreqdo('" . $shipping_line . "') as nomor";
        $query_func     = $this->db->query($sql_func);
        $array_func     = $query_func->row_array();
        $req_no         = $array_func['nomor'];
        ...
        // for container detail
        if ($request['cargo_type'] == "1" || $request['cargo_type'] == "2") 
        {
        ...
        if (is_array($request)) 
        {
            ...
            if ($execHeader) 
            {
                ...
                //mengecek type cargo
                if ($request['cargo_type'] == "1" or $request['cargo_type'] == "2") 
                {
                    for ($i = 0; $i < sizeof($no_container); $i++) 
                    {
                        if ($no_container[$i] != "") 
                ...
                for ($i = 0; $i < sizeof($jenis_dok); $i++) {
                    if ($jenis_dok[$i] != "") {
                        $date_dok = date('Y-m-d', strtotime($tgl_dok[$i]));
                        $sqldok = "INSERT INTO tblreqdo_dok (id_reqdo_header,jenis_dok,no_dok,tgl_dok,filepath_dok,filepath_dok_meta) values ('" . $LastId . "','" . $jenis_dok[$i] . "','" . $no_dok[$i] . "','" . $date_dok . "','" . $file_dok[$i] . "','" . $file_dok[$i] . "')";
                        $execDok = $this->db->query($sqldok);
                    }
                }
                ...
                $execstatus = $this->db->query($sqlStatus);
                $return = "msg#success#Send Request Successful#" . site_url() . '/';
            }
        }
    ...
    public function ExecExtend_BK05082021()
    {
        $action = $this->input->post('action');
    ...
    public function getDataStatusRelease($no_bl, $tgldoakhir, $tglreq)
    {
        if ($tgldoakhir != "") {
            ...
            if ($tglreq != "") {
                ...
                $sql = "select id_reqdo_header from tblreqdo_header where  nobl='$no_bl' and DATE(tglreqdo)='$tglrequest'";
            } else {
    ...
    public function getDataStatusSP2_NonContainer($no_bl, $tglreq)
    {
        $tglreqdo = str_replace('%20', " ", $tglreq);
        $sql = "select id_reqsp2ikt_header from tblreqsp2ikt_header where nosp2_request='$no_bl' and date(wk_status_return)='$tglreqdo'";
        $query = $this->db->query($sql);
        $row = $query->row_array();
        $id_reqsp2ikt_header = $row['id_reqsp2ikt_header'];

        // $sql2 = "select A.*, B.* from tblreqdo_status A join tbltab B on A.kode_status=B.kdnsw and B.kdtab='D4' where id_reqsp2ikt_header='$id_reqsp2ikt_header'";
        $sql2 = "select B.URAIAN, to_char(A.wk_status, 'YYYY-MM-DD HH24:MI:SS') AS CREATED_DATE from tblreqsp2ikt_status A join tbltab B on A.status_code=B.kdnsw and B.kdtab='D4' where id_reqsp2ikt_header='$id_reqsp2ikt_header' ORDER BY A.id_reqsp2ikt_status DESC";
        $query2 = $this->db->query($sql2);
        $hasil = $query2->result_array();
        return $hasil;
    }
    ...
    public function getDataStatusReleaseSP2_NonContainer($no_bl, $tglreq)
    {
        $tglreqdo = str_replace('%20', " ", $tglreq);
        $sql = "select id_reqsp2ikt_header from tblreqsp2ikt_header where nosp2_request='$no_bl' and date(wk_status_return)='$tglreqdo'";
        $query = $this->db->query($sql);
    ...
    public function getDataPdf($id)
    {
        ...
        $sql = " SELECT A.*,B.uredi as loading,D.uredi as discharge,C.uredi as destination,E.uredi as negara_muat,F.uredi as negara_bongkar,G.uredi as negara_tujuan,H.uraian from tblreqdo_header A
        ...
        $arrCargoType = $arrDataHDR["cargo_type"];
        ...
        $sqlGoods = "SELECT * FROM tblreqdo_goods tg natural join tblreqdo_measurement tm where tg.id_reqdo_header='$id'";
        $queryGoods = $this->db->query($sqlGoods);
        $arrDataGoods = array();
        $arrGoods = $queryGoods->result_array();
        ...
        if ($arrCargoType != 3) {
            ...
            foreach ($arrGoods as $goodsArr) {
                array_push($arrDataCon, $goodsArr);
            }
        }
    ...
    public function ExecCoba()
    {
        ...
        if ($_FILES['test_file']['name'] != "") {
            ...
            if ($this->upload->do_upload('test_file')) {
                $data = $this->upload->data();
                $file_name = $data['file_name'];
                $url = $this->config->item('meta_documents');
                // $url = "10.239.7.81/engMetadata/engine/file/updatemetadata";
                $postData = array(
    ...
    public function getDataFormMeasurement($id_reqdo_header)
    {
        ...
        $this->db->select('*');
        $this->db->from('nswdb1.tblreqdo_measurement');
        $this->db->where('id_reqdo_header', $id_reqdo_header);
        $hasil = $this->db->get()->result_array();
        // print_r($data_seal);die('test');
