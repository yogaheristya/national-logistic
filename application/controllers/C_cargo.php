...
class C_cargo extends CI_Controller
{
    ...
    public function getFormExtend_BK05082021($id_reqdo_header)
    {
    ...
    public function getTableRequestSP2ikt()
    {
        ...
        foreach ($dataRelease as $items) {
            // Get BL Number
            $request_announce = $items->request_announce;
            $blInfo = json_decode($request_announce);
    ...
    public function cek_equipment()
    {
        if ($this->session->userdata('group') == '1200') {
            if ($_POST) {
                $no_bl = $_POST['nobl'];

                $this->db->select('id_reqdo_header');
                $this->db->where('nobl', $no_bl);
                $this->db->where('cargo_type', '3');
                $data_header = $this->db->get('tblreqdo_header')->row_array();

                if ($data_header) 
                {
                    $this->db->where('bl_number', $no_bl);
                    $data_equipment = $this->db->get('tblreqdo_equipment')->num_rows();

                    if ($data_equipment == 0) 
                    {
                        $this->db->where('id_reqdo_header', $data_header['id_reqdo_header']);
                        $this->db->where('jenis_dok', '10');
                        $data_dok = $this->db->get('tblreqdo_dok')->row_array();

                        if ($data_dok) 
                        {
                            $file_path = "../upload/dokumen/" . $data_dok['filepath_dok'];
                            
                            if (file_exists($file_path)) 
                            {
                                $doc_vin = fopen($file_path, "r");
                                while(!feof($doc_vin))
                                {
                                    $data_vin = fgetcsv($doc_vin, 0, ',');

                                    for($i=0; $i < count($data_vin); $i++)
                                    {
                                        $insert_data = array(
                                            'id_reqdo_header' => $data_header['id_reqdo_header'],
                                            'bl_number' => $no_bl,
                                            'equipment_identification_number' => $data_vin[$i]
                                        );

                                        if ($insert_data['equipment_identification_number']) {
                                            $this->db->insert('tblreqdo_equipment', $insert_data);
                                        }
                                    }
                                }
                                echo "Data Equipment Berhasil Ditambahkan";
                            } 
                            else 
                            {
                                echo "Dokumen VIN Tidak Ditemukan / Tersedia Dalam Sistem";
                            }
                        } 
                        else 
                        {
                    ...
                    {
                        echo "Data Equipment Sudah Tersedia";
                    }
                } 
                else 
                {
                    echo "NOMOR BL Tidak Ditemukan";
                }
            } else {
                $this->load->view('cek_equipment');
            }
        } else {
